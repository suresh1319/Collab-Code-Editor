...
function getModeFromFilename(name = "") {
  ...
  const modeMap = {
    ...
    ts: { name: "typescript", json: false },
    tsx: { name: "typescript", json: false },
...
const EditorPage = () => {
            ...
            (t) => (
              <div>
                ...
                <div style={{ display: "flex", gap: "10px" }}>
                  <button
                    onClick={() => {
                      socketRef.current.emit(ACTIONS.APPROVE_CODE_EDIT, {
                        roomId,
                        requesterSocketId,
                      });
                      toast.dismiss(t.id);
                  ...
                  <button
                    onClick={() => {
                      socketRef.current.emit(ACTIONS.DENY_CODE_EDIT, {
                        roomId,
                        requesterSocketId,
                      });
                      toast.dismiss(t.id);
          ...
          setActiveFileId((cur) => {
            if (cur && !fs[cur]) {
              ...
              if (root && root.children) {
                const firstFile = root.children.find(
                  (id) => fs[id]?.type === "file",
                );
  ...
  async function handleUploadFiles(e) {
    ...
    socketRef.current?.emit(
      ACTIONS.FS_UPLOAD_BATCH,
      { roomId, nodes: nodesToCreate, fileContents },
      ({ success, message }) => {
        if (success) {
          toast.success(
          ...
          results.forEach(({ fileId }) => {
            delete initialContentsRef.current[fileId];
          });
        }
      },
    );
                ...
                <div className="editor-empty-inner">
                  ...
                  {canWrite && (
                    <p className="editor-empty-hint">
                      Use + buttons in the explorer to create files &amp;
                      folders
                    </p>
                  )}
                </div>
